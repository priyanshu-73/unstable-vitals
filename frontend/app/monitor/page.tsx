...
export default function ExerciseTracker() {
  ...
  const movementHistoryRef = useRef<
    Array<{ x: number; y: number; z: number; time: number; scale: number }>
  >([]);
  ...
  const showToastWithCooldown = (
    type: string,
    message: string,
    category: keyof typeof activeToastIdsRef.current,
    duration = 5000
  ) => {
    ...
    switch (type) {
      ...
      case "warning":
        toast(message, {
          ...toastConfig,
          icon: "⚠️",
          style: { ...toastConfig.style, background: "#F59E0B", color: "#fff" },
        });
        break;
      case "success":
      ...
      case "info":
        toast(message, {
          ...toastConfig,
          icon: "💙",
          style: { ...toastConfig.style, background: "#3B82F6", color: "#fff" },
        });
        break;
    }
  };
  ...
  const calculateAngle = (pointA: any, pointB: any, pointC: any) => {
    ...
    const angleRad = Math.acos(
      Math.max(-1, Math.min(1, dotProduct / (magnitudeBA * magnitudeBC)))
    );
    return (angleRad * 180) / Math.PI;
  };

  // Smoothed angle calculation with moving average
  const calculateSmoothedAngle = (
    pointA: any,
    pointB: any,
    pointC: any,
    angleKey: string
    ...
    const smoothedAngle =
      angleHistoryRef.current[angleKey].reduce((sum, a) => sum + a, 0) /
      angleHistoryRef.current[angleKey].length;
  ...
  const detectEmotion = (faceBlendshapes: any) => {
    ...
    const currentTime = Date.now();
    if (currentTime - lastEmotionToastRef.current < EMOTION_TOAST_COOLDOWN)
      return;
    ...
    const painScore =
      ((eyeSquintLeft + eyeSquintRight) * 1.5 +
        (browDownLeft + browDownRight) * 1.2 +
        mouthPucker) /
      5;
    const frustrationScore =
      ((browDownLeft + browDownRight) * 1.5 +
    ...
    if (painScore > 0.35) {
      ...
      showToastWithCooldown(
        "warning",
        "💪 Stay strong! Every rep brings progress!",
        "emotion",
        EMOTION_TOAST_COOLDOWN
      );
      lastEmotionToastRef.current = currentTime;
      ...
      showToastWithCooldown(
        "info",
        "🌸 Rest is part of progress. Listen to your body!",
        "emotion",
        EMOTION_TOAST_COOLDOWN
      );
      lastEmotionToastRef.current = currentTime;
  ...
  const handleAccidentDetection = (personVisible: boolean) => {
    ...
    if (personVisible) {
      ...
      if (!personCurrentlyVisibleRef.current) {
        ...
        if (
          accidentDetectionActiveRef.current &&
          !emergencyAlertShownRef.current
        ) {
      ...
      if (
        personCurrentlyVisibleRef.current ||
        !accidentDetectionActiveRef.current
      ) {
      ...
      if (accidentDetectionActiveRef.current) {
        const timeSinceDisappeared =
          currentTime - accidentDetectionStartTimeRef.current;
        const secondsRemaining = Math.ceil(
          (ACCIDENT_WARNING_DURATION - timeSinceDisappeared) / 1000
        );
  ...
  const trackBodyMovement = (landmarks: any[]) => {
    ...
    const centerX =
      keyPoints.reduce((sum, p) => sum + p.x, 0) / keyPoints.length;
    const centerY =
      keyPoints.reduce((sum, p) => sum + p.y, 0) / keyPoints.length;
    const centerZ =
      keyPoints.reduce((sum, p) => sum + (p.z || 0), 0) / keyPoints.length;
    ...
    if (isMoving) {
      ...
      if (
        timeSinceLastMovement > STUCK_ACCIDENT_TIME &&
        !emergencyAlertShownRef.current
      ) {
        triggerEmergencyAlert("NOT_MOVING");
      } else if (
        timeSinceLastMovement > STUCK_WARNING_TIME &&
        !stuckWarningShownRef.current
      ) {
        stuckWarningShownRef.current = true;
        const secondsRemaining = Math.ceil(
          (STUCK_ACCIDENT_TIME - timeSinceLastMovement) / 1000
        );
  ...
  const callEmergency = async () => {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/session/emergency`;
  ...
  const triggerEmergencyAlert = (
    type: "PERSON_DISAPPEARED" | "NOT_MOVING" | "FALL_CONFIRMED"
  ) => {
  ...
  const checkShoulderPress = (landmarks: any[]) => {
    ...
    const pose = landmarks[0];
    const leftShoulderAngle = calculateSmoothedAngle(
      pose[23],
      pose[11],
      pose[13],
      "leftShoulder"
    );
    const rightShoulderAngle = calculateSmoothedAngle(
      pose[24],
      pose[12],
      pose[14],
      "rightShoulder"
  ...
  const checkSquats = (landmarks: any[]) => {
    ...
    const pose = landmarks[0];
    const leftKneeAngle = calculateSmoothedAngle(
      pose[23],
      pose[25],
      pose[27],
      "leftKnee"
    );
    const rightKneeAngle = calculateSmoothedAngle(
      pose[24],
      pose[26],
      pose[28],
      "rightKnee"
    ...
    if (hasError) {
      ...
      if (consecutiveFramesWithErrorRef.current >= FRAMES_BEFORE_ALERT) {
        if (currentTime - lastAlertTimeRef.current > ALERT_COOLDOWN) {
          ...
          showToastWithCooldown(
            "error",
            "⚠️ Knees caving in! Risk of injury",
            "formError",
            ALERT_COOLDOWN
          );
          lastAlertTimeRef.current = currentTime;
  ...
  const checkBicepCurls = (landmarks: any[]) => {
    ...
    const leftElbowAngle = calculateSmoothedAngle(
      pose[11],
      pose[13],
      pose[15],
      "leftElbow"
    );
    const rightElbowAngle = calculateSmoothedAngle(
      pose[12],
      pose[14],
      pose[16],
      "rightElbow"
    ...
    if (hasError) {
      ...
      if (consecutiveFramesWithErrorRef.current >= FRAMES_BEFORE_ALERT) {
        if (currentTime - lastAlertTimeRef.current > ALERT_COOLDOWN) {
          ...
          showToastWithCooldown(
            "error",
            "⚠️ Stop swinging! Pin elbows to sides",
            "formError",
            ALERT_COOLDOWN
          );
          lastAlertTimeRef.current = currentTime;
  ...
  const updateDebugData = (landmarks: any[], exerciseName: string) => {
    ...
    switch (exerciseName) {
      case "Shoulder Press":
        ...
        data = {
          ...data,
          elbowAngle: avgElbowAngle.toFixed(0) + "°",
          state: avgElbowAngle > 150 ? "UP ⬆️" : "DOWN ⬇️",
        };
        break;
      ...
      case "Squats":
        ...
        data = {
          ...data,
          kneeAngle: avgKneeAngle.toFixed(0) + "°",
          state: avgKneeAngle < 140 ? "DOWN ⬇️" : "UP ⬆️",
        };
        break;

      case "Bicep Curls":
        const leftBicepAngle = calculateSmoothedAngle(
          pose[11],
          pose[13],
          pose[15],
          "leftBicepDebug"
        );
        const rightBicepAngle = calculateSmoothedAngle(
          pose[12],
          pose[14],
          pose[16],
          "rightBicepDebug"
        ...
        data = {
          ...data,
          bicepAngle: avgBicepAngle.toFixed(0) + "°",
          state: avgBicepAngle < 60 ? "UP ⬆️" : "DOWN ⬇️",
        };
        break;
    }
  ...
  const trackRepSpeed = (landmarks: any[], exerciseName: string) => {
    ...
    switch (exerciseName) {
      case "Shoulder Press":
        currentAngle =
          (calculateSmoothedAngle(pose[11], pose[13], pose[15], "shoulderRep") +
            calculateSmoothedAngle(
              pose[12],
              pose[14],
              pose[16],
              "shoulderRep2"
      ...
      case "Squats":
        currentAngle =
          (calculateSmoothedAngle(pose[23], pose[25], pose[27], "kneeRep") +
            calculateSmoothedAngle(pose[24], pose[26], pose[28], "kneeRep2")) /
          2;
        minAngle = 80;
        maxAngle = 175;
      ...
      case "Bicep Curls":
        currentAngle =
          (calculateSmoothedAngle(pose[11], pose[13], pose[15], "bicepRep") +
            calculateSmoothedAngle(pose[12], pose[14], pose[16], "bicepRep2")) /
          2;
        minAngle = 35;
        maxAngle = 165;
    ...
    if (
      currentRepState !== lastRepStateRef.current &&
      ...
      const consistentState =
        recentHistory.filter((s) => s === currentRepState).length >= 2;
      ...
      if (currentRepState === expectedNextState) {
        ...
        if (!repInProgressRef.current) {
          ...
          if (exerciseName === "Bicep Curls") {
            repCompletionRequiredStateRef.current =
              currentRepState === "contracted" ? "extended" : "contracted";
          } else if (exerciseName === "Shoulder Press") {
            repCompletionRequiredStateRef.current =
              currentRepState === "extended" ? "contracted" : "extended";
          } else if (exerciseName === "Squats") {
            repCompletionRequiredStateRef.current =
              currentRepState === "standing" ? "squatting" : "standing";
          }
        } else {
          ...
          if (repDuration >= MIN_REP_DURATION) {
            ...
            if (currentTime - lastSpeedWarningTimeRef.current > 8000) {
              showToastWithCooldown(
                "warning",
                "⏱️ Slow down! Quality over speed",
                "speed",
                8000
              );
  ...
  const enableWebcam = async () => {
    ...
    const payload = {
      userId,
      exerciseName: exerciseParam,
    };

    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/session`;
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Form + Emotion + Accident Detection
                </p>
              </div>
            </div>
                ...
                <button
                  ...
                  className={`relative overflow-hidden rounded-xl p-6 transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed ${
                    currentExercise === exercise.name
                      ? `bg-gradient-to-r ${exercise.color} shadow-2xl ring-4 ring-white/50`
              ...
              <div className="bg-black/60 backdrop-blur-sm rounded-2xl border border-green-500/30 p-6 shadow-2xl font-mono">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div
                    className={`w-2 h-2 rounded-full ${
                      accidentWarning ? "bg-orange-500" : "bg-green-500"
                    } animate-pulse`}
                  ></div>
                </div>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-bold ${
                        parseFloat(debugData.trunkAngle) > 45
                          ? "text-red-500"
                          : "text-green-400"
                      }`}
                    >
